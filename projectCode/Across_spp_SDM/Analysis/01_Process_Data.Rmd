...
#Renaming DomainID column - conus already has DomainID column
phe.sh = phe.sh %>% 
...
phe.conus = phe.int %>%
  select(-c(domain_id, OBJECTID, Shape_Leng, DomainName, Shape_Le_1, Shape_Area)) %>%
  rename(domain_id = DomainID) %>% #Renaming 
  as.data.frame()
```
...
phe.conus = phe.conus %>%
  ...
  dplyr::select(latitude, longitude, day_of_year, year_rect, native_status, 
                species, phenophase_status, data_name, domain_id, observation_date) 
