...
#This script also bases its working directories off of the super computing directories 
...
#####################################
#1. List .tif files 
tif.files.h = list.files(path = getwd(),
                         pattern = "_EMproj_H_Range\\.tif$",
                         full.names = TRUE)
tif.files.c = list.files(path = getwd(),
                         pattern = "EMproj_C_Range\\.tif$",
                         full.names = TRUE)
...
conus <- vect(file.path("conus.shp")) 
...
pdf(file = file.path(output, paste0("AllSpecies_EMProj_diff(Current-Historic)_Range.pdf")),
    width = 8, height = 7)
for(i in sp){
  ...
  # Save raster to file
  out_path <- file.path(output, paste0(gsub(" ", "_", i), "_EMProj_diff_Range.tif"))
...
names(em.dif.rasters) <- gsub("_Range\\.tif$", "", 
                              basename(tif_files))
...
write.csv(em.dif.df,
          file = file.path(output, "AllSpecies_EMProj_diff_Range.csv"),
          row.names = FALSE)
