...
#rgdal - mainly for data wrangling & plotting/mapping
...
system.time({
  for(i in sp) { #START of foreach loop
	  ...
	  message(paste("CRS already set for", myRespName.h, ":", crs(sp.clim.rasters.h[[myRespName.h]])))
          ...
          myBiomodEM.h <- biomod2::BIOMOD_EnsembleModeling(bm.mod = myBiomodModelOut.h, #singles model output
                                                           models.chosen = bestmodsfullnames.h, #vector of best models
                                                           em.by = 'all', #what models will be combined to ensemble
                                                           em.algo = c('EMwmean'), #types of ensembles models to be computed 
                                                           metric.select = c('TSS'),
          ...
          saveRDS(myBiomodEM.h, file = file.path(L2.em,
                                                 paste0(gsub("\\.", "_", myBiomodEM.h@sp.name),
                                                        "_myBiomodEM_h_", today, ".rds")))
          
          #Save outputs & plots -- Failed on the last species for some reason 
          # pdf(file = file.path(L2.em,
          #                      paste0(gsub("\\.", "_", myBiomodEM.h@sp.name),
          ...
          sink(file = file.path(L2.em, paste0(gsub("\\.", "_", myBiomodEM.h@sp.name),
                                              "_myBiomodEM_h", "_output_", today, ".txt")))
          print(gsub("\\.", "_", myBiomodEM.h@sp.name))
          ...
          sp_projection.h <- biomod2::BIOMOD_EnsembleForecasting(myBiomodEM.h, #output from ensemble 
	 ...
	 print(sp_projection.h@proj.out@link)
	  #read raster output
          r = try(rast(sp_projection.h@proj.out@link))
	...
	#write ouput with new name -- will be overwritten by the current timeperiod model since the species share the same name
          try(writeRaster(r,
                      file = file.path(L2.em, paste0(gsub("\\.", "_", sp_projection.h@models.projected),
                                                     "_EMproj_H_Phenology.TIFF")), overwrite = TRUE))
...
system.time(
  foreach(i = sp, .packages = c("biomod2", "terra", "tools", 
	  ...
	  #Ensure there is a crs proj
          ...
          ###########################
          #Model ensemble models
          myBiomodEM.c <- biomod2::BIOMOD_EnsembleModeling(bm.mod = myBiomodModelOut.c, #singles model output
          ...
          # project species - will pick best one
          sp_projection.c <- biomod2::BIOMOD_EnsembleForecasting(myBiomodEM.c, #output from ensemble 
          ...
          #read raster output 
          try(r = rast(EMproj.c@proj.out@link))
          #write ouput with new name -- will be overwritten by the current timeperiod model since the species share the same name
          try(writeRaster(r,
                      file = file.path(L2.em, paste0(gsub("\\.", "_", sp_projection.c@models.projected),
                                                     "_EMproj_C_Phenology.TIFF")), format = "GTiff", overwrite = TRUE))
          #convert raw data into spatvector points
          d.v = vect(d, geom = c("longitude", "latitude"), keep = TRUE, crs= crs(r))
          #extract raster information using the raw data
          r.d = extract(r, d.v, bind = TRUE)
