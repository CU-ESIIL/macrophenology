...
sp = names(phe.h[94:length(phe.h)])
...
system.time({
  ...
  nb.p = sum(phe.h[, myRespName.h] == 1, na.rm = TRUE)
  ...
  sp_projection.h <- tryCatch({biomod2::BIOMOD_EnsembleForecasting(myBiomodEM.h, #output from ensemble 
...
#}
  ...
  writeRaster(r,
              file = file.path(L2.em, paste0(gsub("\\.", "_", sp_projection.h@models.projected),
                                             "_EMproj_H_Phenology.tif")), overwrite = TRUE)
...
system.time({
  for(i in sp) { #START of foreach loop
    ...
    #sample size of presences 
    nb.p = sum(phe.c[, myRespName.c] == 1, na.rm = TRUE)
