...
sp = names(rang.c[3:length(rang.c)])
...
system.time({
  for(i in sp) { #START of for loop
    ...
    myResp.c = rang.c[, myRespName.c]
    #Making sure there are no 0s
    myResp.c = ifelse(myResp.c == 1, 1, NA)
    #Get corresponding coordinates
    myRespXY.c = as.data.frame(rang.c[, c('longitude', 'latitude')]) #Make sure long goes first!
    ...
    nb.p = sum(rang.c[, myRespName.c] == 1, na.rm = TRUE)
    
    # Set PA.nb.rep conditionally
    if (nb.p < 1000) {
      pa.nb.rep <- 10
    } else {
      pa.nb.rep <- 2
    ...
    sink(file = file.path(L2.em, paste0(gsub("\\.", "_", myBiomodEM.c@sp.name),
                                        "_myBiomodEM_c_", "_output_", today, ".txt")))
    print(gsub("\\.", "_", myBiomodModelOut.c@sp.name))
    ...
    sp_projection.c <- tryCatch({BIOMOD_EnsembleForecasting(myBiomodEM.c, #output from ensemble
          ...
          message("Ensemble Forecasting failed for ", myRespName.c, ": ", e$message)
          return(NULL)
        })
    ...
    writeRaster(r,
                file = file.path(L2.em, paste0(gsub("\\.", "_", sp_projection.c@models.projected),
                                               "_EMproj_C_Range.tif")), overwrite = TRUE)
