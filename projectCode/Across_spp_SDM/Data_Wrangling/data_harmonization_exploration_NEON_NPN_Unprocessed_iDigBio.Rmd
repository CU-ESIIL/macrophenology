...
df = df %>%
  ...
  mutate(species = soi, site_id = NA, state = NA, individual_id = NA, 
         ...
         phenophase_status = NA, data_name = "iDigBio", DomainID = NA) %>%
...
# write.csv(phe.all, file = "FocalSpp_OpenFlowers_NeonNpnHerb_iDigBio.csv", row.names = FALSE)
```
...
#all species & years
phe.all %>% 
  filter(phenophase_status != 0,year_rect>=1910) %>%
  count(species)

#historical
phe.all %>% 
  filter(phenophase_status != 0, year_rect>=1910, year_rect<=1950) %>%
  count(native_status)
#current
phe.all %>% 
  filter(phenophase_status != 0, year_rect>=2009) %>%
  count(native_status)
```
...
phe.all %>%
  #remove any inactive phenophase status (i.e. 0s) - want to be conservative and only view sample sizes for active flowering status
  #Also, only keep observations since the 1910s 
  filter(phenophase_status != 0, year_rect>=1910, species ==  soi) %>%
...
mappin <- function(soi, genus){ #START of map function 
  ...
  #Also, only keep observations since 2009 (start of NEON/NPN)
    filter(phenophase_status != 0, year_rect>=2009, species == soi)
    ##Plotting - Current 
    #base later is domain map
