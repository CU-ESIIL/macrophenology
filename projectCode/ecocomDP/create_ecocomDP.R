...
create_ecocomDP <- function(path,
  ...
  eml <- EDIutils::api_read_metadata(source_id, environment="staging")
  ...
  pheno <- data$`phenology_subset_ecocomDP.csv`
  
  # Replace "NA" and "-9999" values in pheno$update_datetime with actual NA.
  # Normally this is handled by EDIutils, but is not working at the moment.
  pheno$update_datetime[pheno$update_datetime == "NA"] <- NA
  pheno$update_datetime[pheno$update_datetime == "-9999"] <- NA
  
  # Convert date and time columns
  pheno$update_datetime <- ymd_hms(pheno$update_datetime)
  pheno$observation_date <- ymd(pheno$observation_date)
  ...
  # month and year
  flat$event_id <- flat %>% group_by(
    month = floor_date(flat$observation_datetime, "month"),
    year = year(flat$observation_datetime)) %>% group_indices()
  flat <- flat %>% arrange(event_id)
  
  # Assuming observations are made at unique combinations of latitude and 
  # longitude.
  flat$location_id <- flat %>% group_by(latitude, longitude) %>% group_indices()
  
  # Add columns for the location table ----------------------------------------
  
  # Note: Couldn't find a location_name. Using location_id instead.
  
  # Add columns for the taxon table -------------------------------------------
  
  names(flat)[names(flat) == "species"] <- "taxon_name"
